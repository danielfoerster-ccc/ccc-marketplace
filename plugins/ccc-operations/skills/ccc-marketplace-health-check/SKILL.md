...
- The `## Rules` heading check is **case-insensitive**. Known variants: `## Rules`, `## RULES`, `## Rules (Update When Things Go Wrong)`, `## RULES (Failure Modes)`, `## Rules & Anti-Patterns`, `## Rules (Update This Section When Things Go Wrong)`. All pass. Use `re.search(r'^##\s+rules', line, re.IGNORECASE)` — never a bare string match. `**Rules section**` in prose does not count.
- The `## Self-Improvement` heading check is also case-insensitive: `## Self-Improvement`, `## SELF-IMPROVEMENT`, `## Self Improvement` all pass. Use `re.search(r'^##\s+self.improvement', line, re.IGNORECASE)`.
