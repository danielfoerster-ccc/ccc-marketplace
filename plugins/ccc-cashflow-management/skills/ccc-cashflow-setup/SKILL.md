...
Read `references/account-opening-guide.md` for the full step-by-step bank application process. Summary:

1. **Gather documents** — ID, proof of address, business registration, tax ID
2. **Submit primary bank application** — 5 accounts; note confirmation number and activation timeline (Holvi: 24h, N26: 2–3 days, Sparkasse: 5–10 days)
3. **Submit secondary bank application** — savings only, no debit cards
4. **Create Account Ledger** — use the table in the reference file; populate bank, purpose, card assignment, status for all 7+ accounts

---

## PHASE 3: ACCOUNT VERIFICATION & SETUP (Days 11–14)

Read `references/account-opening-guide.md` for the full verification checklist. Summary:

1. **Confirm each account** — log in, verify IBAN, test with €0.01 micro-transfer, update Account Ledger status to Active
2. **Configure online banking** — spending limits (Profit/Owner Comp Mastercards: €500–1,000/day), transaction alerts (OpEx low-balance, any Profit/Tax withdrawal)
3. **Assign Mastercards** — OpEx → Finance/Ops, Owner Comp → Owner, Profit → Owner locked away; document in Account Ledger
4. **Run verification checklist** — all 7+ accounts Active before moving to Phase 4
