...
**Critical:** Use Quarter 1 rollout percentages from Instant Assessment, NOT full-term TAPs. Full step-by-step guide with examples: `references/first-allocation-guide.md`

**5 steps:**

1. **Gather Current Balance** — total cash across all accounts. Zero balance? Still open all accounts now — structure matters more than starting money. See reference for debt-service edge case.
2. **Set Allocation Percentages** — use Instant Assessment Q1 percentages. Standard split: Profit 5%, Owner Comp 50%, Tax 15%, OpEx 30%. With Mat&Sub: OpEx drops to 20%, Mat&Sub gets 10%.
3. **OpEx Safety Check (non-negotiable)** — confirm OpEx covers ≥14 days of committed costs. If short: reduce Profit to 2–3%, increase OpEx, document the conservative choice.
4. **Execute Transfers** — from Income account to each sub-account with clear reference memos. Wait for clearing (1–2 business days), verify receipt.
5. **Document First Allocation** — complete the First Allocation Receipt (template in reference file).
